...
class JadwalService {
  ...
  Future<List<Jadwal>> getJadwalHariIni() async {
    ...
    final response = await http.get(
      Uri.parse('${Config.baseUrl}/jadwal-hari-ini'),
      headers: headers,
    );
  ...
  Future<List<Riwayat>> getRiwayat() async {
    final headers = await _getHeaders();
    final response = await http.get(
      Uri.parse('${Config.baseUrl}/riwayat-mengajar'),
      headers: headers,
    );
  ...
  // 1b. GET JADWAL PIKET (Semua jadwal yang sedang berlangsung)
  // --- REVISI: Tambahkan parameter opsional {int? kelasId} ---
  Future<List<Jadwal>> getJadwalPiket({int? kelasId}) async {
    final headers = await _getHeaders();

    // Logika URL dinamis: Jika ada kelasId, tambahkan query parameter ?kelas_id=...
    String url = '${Config.baseUrl}/jadwal-piket';
    if (kelasId != null) {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(body['message'] ?? 'Gagal memuat jadwal piket');
    }
  }
  ...
  Future<List<Kelas>> getDaftarKelas() async {
    final headers = await _getHeaders();
    final response = await http.get(
      Uri.parse('${Config.baseUrl}/kelas'),
      headers: headers,
    );
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      List<dynamic> listJson = data['data'];
      return listJson.map((json) => Kelas.fromJson(json)).toList();
    } else {
      throw Exception('Gagal mengambil daftar kelas');
    }
  }

  // 2. Ambil jadwal piket (tambahkan parameter opional kelasId)
  ...
  Future<List<PresensiDetail>> getDetailSiswa(int jurnalId) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Gagal memuat detail siswa (Code: ${response.statusCode})',
      );
    }
  }
